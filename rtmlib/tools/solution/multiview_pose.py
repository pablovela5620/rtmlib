...
class MultiviewBodyTracker:
    ...
    def __call__(
        ...
        if self.cams_for_detection_idx is not None:
            Pall: Float32[ndarray, "nViews 3 4"] = np.array([Pall[i] for i in self.cams_for_detection_idx])
        ...
        for image_idx, bgr in enumerate(bgr_list):
            try:
                bboxes: Float32[ndarray, "num_dets 4"] = bboxes_extrap[image_idx][None, :]
            except NameError:
    ...
    def extrapolate_3d_keypoints(self, kpts_3d_t1, kpts_3d_t2) -> Float32[ndarray, "num_kpts 4"]:
        """
